...
class MonitorAccount(Base):
    ...
    id = Column(Integer, primary_key=True, comment='主键ID')
    owner_user_id = Column(Integer, nullable=True, index=True, comment='监控系统用户ID')
    platform = Column(String(16), nullable=False, index=True, comment='平台: dy, xhs, bili')
    creator_id = Column(String(255), nullable=False, comment='创作者ID')
    display_name = Column(String(255), default='', comment='展示名称')
    avatar_url = Column(Text, default='', comment='头像URL')
    is_active = Column(Integer, default=1, comment='1=启用, 0=暂停')
    created_at = Column(BigInteger, comment='创建时间戳')
    last_crawl_at = Column(BigInteger, comment='最后采集时间戳')
    last_crawl_status = Column(String(32), default='never', comment='never/success/failed/running')


class MonitorUser(Base):
    __tablename__ = 'monitor_user'
    id = Column(Integer, primary_key=True, comment='主键ID')
    email = Column(String(255), nullable=False, unique=True, index=True, comment='登录邮箱')
...
class MonitorSnapshot(Base):
    ...
    id = Column(Integer, primary_key=True, comment='主键ID')
    account_id = Column(Integer, nullable=False, index=True, comment='关联monitor_account.id')
    snapshot_ts = Column(BigInteger, nullable=False, index=True, comment='快照时间戳')
    followers = Column(Integer, default=0, comment='粉丝数')
    total_likes = Column(Integer, default=0, comment='总获赞数')
    total_videos = Column(Integer, default=0, comment='作品总数')
    total_views = Column(Integer, default=0, comment='总播放量')
    latest_video_likes = Column(Integer, default=0, comment='最新作品点赞数')
    latest_video_id = Column(String(255), default='', comment='最新作品ID')
    raw_json = Column(Text, default='{}', comment='扩展指标JSON')
