...
async def find_login_qrcode(page: Page, selector: str, timeout_ms: int = 10000) -> str:
    ...
    try:
        ...
        if not elements:
            utils.logger.info(f"[find_login_qrcode] selector returned None: {selector[:60]}")
            return ""
        ...
        if not login_qrcode_img:
            utils.logger.info(f"[find_login_qrcode] src attribute is empty")
            return ""
        ...
        utils.logger.info(f"[find_login_qrcode] failed: {e}")
        return ""
