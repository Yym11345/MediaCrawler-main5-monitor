...
def run(
    ...
    *,
    ...
    async def _cleanup_with_timeout() -> bool:
        cleanup_task = asyncio.create_task(app_cleanup())
        try:
            await asyncio.wait_for(cleanup_task, timeout=cleanup_timeout_seconds)
            return True
        except asyncio.TimeoutError:
            print(f"[Main] Cleanup timeout ({cleanup_timeout_seconds}s), cancelling remaining cleanup.", flush=True)
            cleanup_task.cancel()
    ...
    async def _runner() -> None:
        ...
        try:
            ...
            try:
                cleanup_completed = await _cleanup_with_timeout()
            except Exception as e:
                print(f"[Main] Error during cleanup: {e}")
            await _cancel_remaining_tasks()

            if not cleanup_completed:
                exit_code = force_exit_code if cancelled else (1 if main_error else 0)
                print(f"[Main] Force exiting after cleanup timeout with code {exit_code}.", flush=True)
                os._exit(exit_code)
