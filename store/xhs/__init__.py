...
def _short_log_text(value: str, limit: int = 80) -> str:
...
async def update_xhs_note(note_item: Dict):
    ...
    utils.logger.info(
        "[store.xhs.update_xhs_note] xhs note saved: "
        f"note_id:{local_db_item.get('note_id')}, "
        f"title:{_short_log_text(local_db_item.get('title'))}, "
        f"likes:{local_db_item.get('liked_count')}, "
        f"comments:{local_db_item.get('comment_count')}, "
        f"shares:{local_db_item.get('share_count')}"
    )
...
async def update_xhs_note_comment(note_id: str, comment_item: Dict):
    ...
    utils.logger.info(
        "[store.xhs.update_xhs_note_comment] xhs note comment saved: "
        f"note_id:{note_id}, comment_id:{comment_id}, likes:{local_db_item.get('like_count')}"
    )
