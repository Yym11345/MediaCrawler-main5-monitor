...
class XiaoHongShuClient(AbstractApiClient, ProxyRefreshMixin):
    ...
    def _is_login_expired_error(exc: Exception) -> bool:
        message = str(XiaoHongShuClient._unwrap_retry_error(exc))
        message_lower = message.lower()
        return (
            "登录已过期" in message
            or "登陆已过期" in message
            or ("login" in message_lower and "expired" in message_lower)
        )
    ...
    async def get_all_notes_by_creator(
        ...
        unlimited_notes = max_notes_count <= 0
        while notes_has_more and (unlimited_notes or len(result) < max_notes_count):
            try:
                ...
                raw_exc = self._unwrap_retry_error(exc)
                if self._is_login_expired_error(exc):
                    utils.logger.warning(
                        f"[XiaoHongShuClient.get_all_notes_by_creator] XHS returned login expired while paging "
                        f"user_id:{user_id}, cursor:{notes_cursor or '<first>'}. Refreshing browser cookies and retrying once."
                    )
                    try:
                        ...
                        utils.logger.warning(
                            f"[XiaoHongShuClient.get_all_notes_by_creator] Stop paging user_id:{user_id}. "
                            f"Kept {len(result)} notes already collected. Last error: {raw_retry_exc}"
                        )
                        break
                else:
                    utils.logger.warning(
                        f"[XiaoHongShuClient.get_all_notes_by_creator] Stop paging user_id:{user_id}. "
                        f"Kept {len(result)} notes already collected. Last error: {raw_exc}"
                    )
                    break
            if not notes_res:
                utils.logger.error(
            ...
            if unlimited_notes:
                ...
                remaining = max_notes_count - len(result)
                if remaining <= 0:
                    break
