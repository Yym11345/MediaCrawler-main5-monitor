...
from tools.browser_profile import get_configured_browser_profile_dir
...
class XiaoHongShuCrawler(AbstractCrawler):
    ...
    def __init__(self) -> None:
        ...
        api_url = "https://webapi.rednote.com" if config.XHS_INTERNATIONAL else "https://edith.xiaohongshu.com"
        self.cookie_urls = [self.index_url, api_url]
    ...
    async def fetch_creator_notes_detail(self, note_list: List[Dict]):
        ...
        for post_item in note_list:
            note_id = post_item.get("note_id") or post_item.get("id", "")
            if not note_id:
                skipped += 1
                continue
            task_list.append(
        ...
        if skipped:
            utils.logger.warning(f"[fetch_creator_notes_detail] Skipped {skipped}/{len(note_list)} notes with empty note_id")
    ...
    async def launch_browser(
        ...
        if config.SAVE_LOGIN_STATE:
            ...
            user_data_dir = str(get_configured_browser_profile_dir(config.PLATFORM))
            utils.logger.info(f"[XiaoHongShuCrawler.launch_browser] Browser profile: {user_data_dir}")
