...
class BilibiliClient(AbstractApiClient, ProxyRefreshMixin):
    ...
    def __init__(
        ...
        *,
        ...
        self.cookie_urls = ["https://www.bilibili.com", "https://api.bilibili.com"]
    ...
    @staticmethod
    def _extract_api_data(data: Dict) -> Any:
        if data.get("code") != 0:
            raise DataFetchError(data.get("message", "unknown error"))
        return data.get("data", {})

    @staticmethod
    def _browser_fetch_headers(method: str, headers: Optional[Dict[str, str]]) -> Dict[str, str]:
        blocked_headers = {
            "accept-encoding",
            "connection",
            "content-length",
        ...
        browser_headers = {"Accept": "application/json, text/plain, */*"}
        for key, value in (headers or {}).items():
            key_lower = key.lower()
            if key_lower in blocked_headers:
                continue
            if key_lower == "content-type" and method.upper() in {"GET", "HEAD"}:
                continue
            browser_headers[key] = value
        return browser_headers

    async def _playwright_request_json(self, method: str, url: str, **kwargs) -> Optional[Dict]:
        try:
            ...
            response = await self.playwright_page.context.request.fetch(
                self._url_with_params(url, kwargs.get("params")),
                **request_options,
            )
            ...
            utils.logger.warning(
                f"[BilibiliClient.request] Playwright request returned non-JSON. "
                f"content: {self._response_snippet(text) if 'text' in locals() else ''}"
            )
            return None
        except Exception as exc:
            utils.logger.warning(f"[BilibiliClient.request] Playwright request fallback failed: {exc}")
            return None

    async def _browser_fetch_json(self, method: str, url: str, **kwargs) -> Optional[Dict]:
        try:
            if not self.playwright_page or self.playwright_page.is_closed():
                return None
            ...
            if body is not None and not isinstance(body, str):
                body = json.dumps(body, separators=(",", ":"), ensure_ascii=False) if isinstance(body, (dict, list)) else str(body)
            ...
            result = await self.playwright_page.evaluate(
                ...
                """,
                {
                    "url": self._url_with_params(url, kwargs.get("params")),
                    "method": method.upper(),
                    "headers": self._browser_fetch_headers(method, kwargs.get("headers")),
                    "body": body,
                },
            )
            ...
            utils.logger.warning(
                f"[BilibiliClient.request] Browser fetch returned non-JSON. "
                f"status_code: {result.get('status') if 'result' in locals() else 'unknown'}, "
                f"content: {self._response_snippet(result.get('text', '')) if 'result' in locals() else ''}"
            )
            return None
        except Exception as exc:
            utils.logger.warning(f"[BilibiliClient.request] Browser fetch fallback failed: {exc}")
            return None
    ...
    async def request(self, method, url, **kwargs) -> Any:
        ...
        async with make_async_client(proxy=self.proxy) as client:
            response = await client.request(method, url, timeout=self.timeout, **request_kwargs)
        try:
            data: Dict = response.json()
        except json.JSONDecodeError as exc:
            snippet = self._response_snippet(response.text)
            utils.logger.warning(
                f"[BilibiliClient.request] HTTP client received non-JSON response. "
                f"status_code: {response.status_code}, content: {snippet}"
            )
            data = await self._playwright_request_json(method, url, **request_kwargs)
            if data is None:
                data = await self._browser_fetch_json(method, url, **request_kwargs)
            if data is None:
                if response.status_code == 412 or "security control policy" in response.text:
                    raise DataFetchError(
                        "Bilibili returned 412 security control page. "
                        "Complete any verification in the opened browser or retry later."
                    ) from exc
                raise DataFetchError(
                    f"Failed to decode JSON. status_code: {response.status_code}, content: {snippet}"
                ) from exc
        return self._extract_api_data(data)
    ...
    async def get_wbi_keys(self) -> Tuple[str, str]:
        ...
        # Try reading from localStorage first (faster)
        try:
        ...
        # Fallback: fetch from API if localStorage didn't work
